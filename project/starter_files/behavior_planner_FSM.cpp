...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  //auto look_ahead_distance = (velocity_mag * lookahead_time) + (accel_mag * lookahead_time * lookahead_time);  
  //auto look_ahead_distance = velocity_mag * _lookahead_time + 0.5 * _max_accel * _lookahead_time * _lookahead_time;
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.velocity.x = _speed_limit * std::cos(goal.rotation.yaw);  // <- Fix This  // <- Fix This
      goal.velocity.y = _speed_limit * std::sin(goal.rotation.yaw);  // <- Fix This  // <- Fix This
      goal.velocity.z = 0;
    }
